...
void insert_registry_sqlite(sqlite3* db, person* p) {
    ...
    if (auto f = dynamic_cast<supplier*>(p)) {
        sqlite3_bind_text(stmt, 1, "Supplier", -1, SQLITE_TRANSIENT);
    ...
    } else if (auto o = dynamic_cast<worker*>(p)) {
        sqlite3_bind_text(stmt, 1, "Worker", -1, SQLITE_TRANSIENT);
    ...
    } else if (auto v = dynamic_cast<seller*>(p)) {
        sqlite3_bind_text(stmt, 1, "Seller", -1, SQLITE_TRANSIENT);
    ...
    } else if (auto e = dynamic_cast<employee*>(p)) {
        sqlite3_bind_text(stmt, 1, "Employee", -1, SQLITE_TRANSIENT);
...
void load_database_sqlite(sqlite3* db, vector<person*>& persons) {
    for (auto p : persons) {
        delete p;
    }
    persons.clear();
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        if (s_type == "Supplier") {
            ...
            persons.push_back(new supplier(s_nome, s_address, s_telefone, credito, divida));
        } else if (s_type == "Employee") {
            ...
            persons.push_back(new employee(s_nome, s_address, s_telefone, sector, sal, imp));
        } else if (s_type == "Admin") {
            int sector = sqlite3_column_int(stmt, 6);
            int sal = sqlite3_column_int(stmt, 7);
            float imp = static_cast<float>(sqlite3_column_double(stmt, 8));
            int allowance = sqlite3_column_int(stmt, 9);
            persons.push_back(new admin(s_nome, s_address, s_telefone, sector, sal, imp, allowance));
        } else if (s_type == "Worker") {
            int sector = sqlite3_column_int(stmt, 6);
            int sal = sqlite3_column_int(stmt, 7);
            float imp = static_cast<float>(sqlite3_column_double(stmt, 8));
            int prod = sqlite3_column_int(stmt, 10);
            int comm = sqlite3_column_int(stmt, 11);
            persons.push_back(new worker(s_nome, s_address, s_telefone, sector, sal, imp, prod, comm));
            ...
            int comm = sqlite3_column_int(stmt, 11);
            persons.push_back(new seller(s_nome, s_address, s_telefone, sector, sal, imp, sales, comm));
        }
    }
    sqlite3_finalize(stmt);
}

bool init_database(const string& db_file, sqlite3*& db, vector<person*>& persons) {
    ...
    if (count == 0) {
        vector<person*> mock_entries;
        mock_entries.push_back(new supplier("User 1", "Address 1", "Number 1", 10000, 2000));
        mock_entries.push_back(new employee("User 2", "Address 2", "Number 2", 10, 5000, 10.0f));
        mock_entries.push_back(new admin("User 3", "Address 3", "Number 3", 10, 8000, 15.0f, 1500));
        mock_entries.push_back(new seller("User 4", "Address 4", "Number 4", 10, 6000, 12.0f, 4000, 10));
    ...
    load_database_sqlite(db, persons);
    return true;
}
...
int main() {
    ...
    init_database(db_file, db, persons);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        static int input_seller_commission = 0;
        
        const char* types[] = { "Supplier", "Employee", "Admin", "Worker", "Seller" };
            ...
            if (ImGui::BeginTabItem("Registry Overview")) {
                if (ImGui::BeginTable("PeopleTable", 8, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg | ImGuiTableFlags_Resizable)) {
                    ...
                    for (size_t i = 0; i < persons.size(); ++i) {
                        ...
                        if (dynamic_cast<supplier*>(p)) role_str = "Supplier";
                        else if (dynamic_cast<admin*>(p)) role_str = "Admin";
                        else if (dynamic_cast<worker*>(p)) role_str = "Worker";
                        else if (dynamic_cast<seller*>(p)) role_str = "Seller";
                        else if (dynamic_cast<employee*>(p)) role_str = "Employee";
                    ...
                    if (to_delete != -1) {
                        delete_registry_sqlite(db, persons[to_delete]->getNome(), persons[to_delete]->getAddress(), persons[to_delete]->getTelefone());
                        delete persons[to_delete];
                        persons.erase(persons.begin() + to_delete);
                    }
                } else {
            ...
            if (ImGui::BeginTabItem("Add New Registry")) {
                ...
                if (selected_type_idx == 0) { // Fornecedor
                    ...
                    if (selected_type_idx == 2) { // Admin
                        ...
                        ImGui::InputInt("Commission (%)", &input_seller_commission);
                    }
                }
                ...
                if (ImGui::Button("Add Registry Entry", ImVec2(180, 40))) {
                    person* new_p = nullptr;
                    if (selected_type_idx == 0) {
                        new_p = new supplier(name_buf, addr_buf, phone_buf, input_credito, input_divida);
                    } else if (selected_type_idx == 1) {
                        new_p = new employee(name_buf, addr_buf, phone_buf, input_sector, input_sal_base, input_imposto);
                        ...
                        new_p = new worker(name_buf, addr_buf, phone_buf, input_sector, input_sal_base, input_imposto, input_prod_val, input_commission);
                    } else if (selected_type_idx == 4) {
                        new_p = new seller(name_buf, addr_buf, phone_buf, input_sector, input_sal_base, input_imposto, input_sales_val, input_seller_commission);
                    }
                    
                    if (new_p) {
                        insert_registry_sqlite(db, new_p);
                        persons.push_back(new_p);
                    }
